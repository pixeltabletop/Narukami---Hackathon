...
test("bloquea la carga de IA en el modo de entrega sin modelos", async () => {
  const prior = process.env.CHEN_ENABLE_QVAC;
  process.env.CHEN_ENABLE_QVAC = "0";
  try {
    ...
    if (prior === undefined) delete process.env.CHEN_ENABLE_QVAC;
    else process.env.CHEN_ENABLE_QVAC = prior;
  }
});
